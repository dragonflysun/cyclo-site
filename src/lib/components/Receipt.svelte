<script lang="ts">
	...
	import { wrappedFlareAddress, erc20PriceOracleReceiptVaultAddress } from '$lib/stores';
	...
	const readableBalance = (parseFloat(receipt.balance) / 1e18).toFixed(2); // Assuming the balance is in wei
...
<Card size="lg">
	<div class="flex w-full flex-col items-center justify-center gap-6">
		...
		>
			...
			<div class="flex flex-row items-center gap-2">
				<a href={$targetNetwork.b}>{truncateEthAddress(tokenAddress)}</span>
			</div>
		</div>
